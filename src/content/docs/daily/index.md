---
title: Today and the daily brief
description: The screen you open each morning — who is arriving, what needs a decision, and the brief written out for you and your team.
sidebar:
  order: 1
---

**Today** is the default screen. It has two halves: the **daily brief**, which is the day written
out in sentences, and the metric tiles underneath, which are the numbers.

![Today. The brief at the top says what needs a decision; the tiles under it are the numbers behind that.](/screens/today.en.png)

## The brief

The brief is generated for your workspace each day, at the **time** and in the **language** you set
under [workspace settings](/setup/workspace/#operations-preferences), and it is refreshed as the day
moves. It is one dense sheet, in this order:

1. **A header scorecard** — revenue and occupancy for the period, against the last one.
2. **AI insights** — one card per guest who needs something, with what happened and what to do.
   Risky checkouts, unanswered questions, a complaint building over three messages, an upsell
   worth offering. Each card links to the exact thread or stay.
3. **Arrivals, in-house, departures** for today.
4. **Cleaning, three days out** — grouped by [cleaning cluster](/daily/tasks/#cleaning-clusters),
   with a per-cluster message ready to paste into WhatsApp.
5. **Ready-to-copy guest messages** for the arrivals and departures that need one.

### Marking it reviewed

Press **Reviewed** when you have been through it. It is a signal to the rest of the team that
someone has looked, not a completion score — nothing is hidden once you do.

### Sharing it

**Share** writes a message for the channel you pick — WhatsApp, email, a cleaner's group — with
today's highlights and a link to the full brief. Each channel gets copy written for it rather than
the same paragraph pasted three times.

The link opens the full brief in Santara AI, so recipients need an account with access to this
workspace. For a cleaner who should not see the estate, share the **cluster message** instead: it
contains only their jobs.

### Language and timing

The brief is written in the workspace's language, set under
[Workspace settings](/setup/workspace/). Its idea of "today" comes from the workspace timezone —
which matters more than it sounds, because a brief generated in the wrong timezone reports
yesterday's checkouts.

## The tiles

Under the brief: arrivals and departures counted, occupancy, revenue for the period, open tasks,
unanswered messages. Each one is a link to the screen behind it.

**Revenue means net payout** — what the channel actually pays you. It is labelled as such wherever
it appears. See [Reports](/money/reports/) for the full picture.

## Notifications

Anything that needs you while you are not looking at this screen produces a notification in the
bell menu. There are eight kinds, and they are the eight things that genuinely change your day:

| | |
| --- | --- |
| **Bookings** | A new booking, a changed booking, a cancellation |
| **Cleaning** | A clean assigned to you, a clean marked finished |
| **Tickets** | A ticket opened, assigned to you, or done |

Notifications are addressed by **who can act on them**, not by name, so a manager added next month
sees the ones that were always meant for their role. Configure what you receive under
[Workspace settings](/setup/workspace/#notifications).
