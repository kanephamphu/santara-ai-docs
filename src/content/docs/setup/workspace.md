...
| **Message Templates** | Shared — saved guest replies |
...
- **The workspace's** language decides what language the [daily brief](/daily/) is written in, and
  the default language for anything the workspace generates for a group.

These are separate on purpose: a Vietnamese-speaking manager can work in Vietnamese inside a
workspace whose brief goes out in Indonesian to the cleaning team.

## Timezone

The workspace timezone decides what "today" means — for arrivals, for cleaning, for the brief, and
for every date on every report. Set it to where the properties are, not where you are.

## Currency

The workspace currency is what figures are displayed in. Your [booking site](/setup/booking-site/)
has its own currency setting for what guests see.

## Operations preferences

The workspace tab carries the settings that change how the product behaves for everyone in it:

| Setting | What it does |
| --- | --- |
| **Daily brief time** | When the [brief](/daily/) is generated each day |
| **Daily brief language** | What language it is written in — separate from the interface language |
| **Workspace timezone** | What "today" means, everywhere |
| **Display currency** | The currency figures are shown in |
...
The bell in the header. Notifications are addressed by **who can act on them** rather than by name,
...
:::note[The first sync is quiet on purpose]
Connecting a channel imports your booking history, sometimes a year of it. Those are not new
bookings and you are not notified about them — only about what happens after you connect.
:::
...
The account menu lists every workspace you belong to. Each has its own properties, team, billing
and booking site; nothing is shared between them.
