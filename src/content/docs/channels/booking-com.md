...
description: Add the connectivity provider in your extranet, enter your Hotel ID, map your rooms, and Santara activates the channel for you.
sidebar:
  order: 3
---
...
Nothing happens in Santara until this is done. The connect dialog has a checkbox confirming you
...
Choose the Santara property this hotel belongs to. The hotel connects **directly to that
...
Santara asks Booking.com for the rooms on that hotel and shows them under **Your rooms**. For each
one, pick the Santara room it is.
...
Then press **Map, activate & sync**. Santara maps the rooms, activates the channel, and syncs
...
| On screen | Meaning |
| --- | --- |
| *Linked — not mapped yet* | Booking.com knows the room; it points at no Santara room |
...
Santara will not guess an occupancy price on a live listing. That refusal is deliberate: a guessed
...
Prices you set in Santara are pushed **out** to Booking.com; nothing is imported **in**. Set your
prices here (or with [pricing rules](/money/pricing/)) and treat Santara as the source of truth.
...
- **Remove from Santara** on one room — removes its imported data here only.
